...
ibz_test_conditional_moves()
{
    ...
    ibz_set(&b, 1);
    c = 1;
    ibz_conditional_assign(&x, &a, &b, c);
    res = res | !(ibz_cmp(&a, &x) == 0);
    res = res | !(ibz_cmp(&a, &ibz_const_two) == 0);
    c = 0;
    ibz_conditional_assign(&y, &a, &b, c);
    res = res | !(ibz_cmp(&b, &y) == 0);
    res = res | !(ibz_cmp(&b, &ibz_const_one) == 0);
    c = 0;
    ibz_conditional_swap(&y, &x, c);
    res = res | !(ibz_cmp(&b, &y) == 0);
    res = res | !(ibz_cmp(&a, &x) == 0);
    c = 1;
    ibz_conditional_swap(&y, &x, c);
    res = res | !(ibz_cmp(&b, &x) == 0);
    res = res | !(ibz_cmp(&a, &y) == 0);

    if (res) {
        printf("Quaternion module integer test ibz_test_conditional_moves failed\n");
    }
    ibz_finalize(&x);
    ibz_finalize(&y);
    ibz_finalize(&a);
...
ibz_test_intbig()
{
    ...
    res = res | ibz_test_conditional_moves();
    return res;
}
