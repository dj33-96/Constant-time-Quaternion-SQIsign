...
quat_lideal_generator(quat_alg_elem_t *gen, const quat_left_ideal_t *lideal, const quat_alg_t *alg)
{
    ...
    for (int i = 0; i < 4; i++) {
        ibz_copy(&((gen->coord)[i]), &(lideal->lattice.basis[i][0]));
    }
    ...
    quat_lideal_create(&test, gen, &(lideal->norm), (lideal->parent_order), alg);
...
int
quat_lideal_generator_old(quat_alg_elem_t *gen, const quat_left_ideal_t *lideal, const quat_alg_t *alg)
{
