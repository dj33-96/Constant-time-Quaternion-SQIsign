...
void ibz_mat_4xn_hnf_mod_core_old(ibz_mat_4x4_t *hnf,
                                  int generator_number,
                                  const ibz_vec_4_t *generators,
                                  const ibz_t *mod);
/** @}
...
/**  @internal
 * @defgroup quat_ct_normeq_helpers Helper functions for the constant-time represent_integer function
 * @{
 */

/** @brief Constant time conditional move
 *
 * Conditional move in constant time. If selector = -1 then load r with a, else if selector = 0 then keep r.
 *
 * This function here is design to only work using GMP large integer arithmetic.
 *
 * @param out Output of the algorithm
 * @param in Input large integer
 * @param sel: Selector bits.  If sel = -1 then load r with a, else if sel = 0 then keep r.
 */
void ibz_ct_cmov(ibz_t *out, const ibz_t *in, int8_t sel);

/**@brief Maximum of 2 values
 *
 * Finds the maximum of two given inputs
 *
 * @param max Output: max(a,b)
 * @param a First input
 * @param b Second input
 */
void ibz_max_2(ibz_t *max, const ibz_t *a, const ibz_t *b);

/**
 *@brief Minimum of 2 values
 *
 * Finds the minimum of two given inputs
 *
 * @param min Output: min(a,b)
 * @param a First input
 * @param b Second input
 */
void ibz_min_2(ibz_t *min, const ibz_t *a, const ibz_t *b);
 ...
 *
 * @param x Output
 * @param M input (for example M in x^2+dy^2=M)
 * @param u second input (for example sqrt(-d) in x^2+dy^2=M)
 * @param l length
 */
void ibz_constant_halfgcd(ibz_t *x, const ibz_t *M, const ibz_t *u, const ibz_t *l);
/** @}
 */
