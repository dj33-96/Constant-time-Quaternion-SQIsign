...
void
ibz_xor(ibz_t *r, const ibz_t *a, const ibz_t *b)
{
    mpz_xor(*r, *a, *b);
}

void
ibz_and(ibz_t *r, const ibz_t *a, const ibz_t *b)
{
    mpz_and(*r, *a, *b);
}
void
ibz_or(ibz_t *r, const ibz_t *a, const ibz_t *b)
{
    mpz_ior(*r, *a, *b);
}
int8_t
ibz_sgn(const ibz_t *a)
{
    int8_t sgn;
    sgn = (mpz_sgn(*a) < 0) ? 1 : 0;
    return sgn;
...
int
is_negative(const ibz_t *op)
{
    return mpz_sgn(*op) < 0 ? 1 : 0;
}
int
...
void
rand_exact_m_bits(ibz_t *result, gmp_randstate_t state, uint32_t m)
{
    mpz_urandomb(*result, state, m); // Random in [0, 2^m)
    mpz_setbit(*result, m - 1);      // Ensure the top bit is 1 → now in [2^(m-1), 2^m)
}
...
// if c then swap else stay same
void
ibz_conditional_swap(ibz_t *a, ibz_t *b, int c)
{
    ibz_t ad, bd;
    ibz_init(&ad);
    ibz_init(&bd);
    ibz_conditional_assign(&ad, b, a, c);
    ibz_conditional_assign(&bd, a, b, c);
    ibz_copy(b, &bd);
    ibz_copy(a, &ad);
    ibz_finalize(&ad);
    ibz_finalize(&bd);
