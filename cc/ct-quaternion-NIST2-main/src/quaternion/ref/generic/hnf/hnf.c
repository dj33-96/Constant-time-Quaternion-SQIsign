...
ibz_mat_4x4_is_hnf(const ibz_mat_4x4_t *mat)
{
    ...
    // check that last nom-zero elements index per column is strictly increasing
    int linestart = -1;
    int i = 0;
    for (int j = 3; j > -1; j--) {
...
ibz_mat_4xn_hnf_mod_core(ibz_mat_4x4_t *hnf, int generator_number, const ibz_vec_4_t *generators, const ibz_t *mod)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        for (j = k + 1; j < n; j++) {
            ...
            // also requires xgcd to output aki, 1, 0 on input aki,0 (and d=1,u=1,v=0 if aki = 0): (means nothing
            // changes if aji = 0)
            ...
            ibz_set(&r, (ibz_is_zero(&coeff_1) & (ibz_is_zero(&(a[j][i])))) != 0);
            ibz_add(&coeff_1, &r, &coeff_1);
            // compute k coefficient in lc for j
            ibz_div(&coeff_2, &r, &(a[j][i]), &d);
            ibz_neg(&coeff_2, &coeff_2);
            // ensure this does nothing if a[j][i]=0
            ibz_vec_4_linear_combination_mod(&(a[j]), &coeff_1, &(a[j]), &coeff_2, &(a[k]), &m); // do lin comb mod m
        ...
        ibz_mod_not_zero(&(w[i][i]), &(w[i][i]), &m);
        for (int h = i - 1; h > -1; h--) {
        ...
        ibz_mod_not_zero(&(a[k][i]), &(a[k][i]), &m);
    }
    for (j = 0; j < 4; j++) {
        for (i = 0; i < 4; i++) {
...
void
ibz_mat_4xn_hnf_mod_core_old(ibz_mat_4x4_t *hnf, int generator_number, const ibz_vec_4_t *generators, const ibz_t *mod)
{
    int i = 3;
    assert(generator_number > 3);
    int n = generator_number;
